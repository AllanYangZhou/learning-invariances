...
class ChannelUniformAug(nn.Module):
    """docstring for MLPAug"""
    def __init__(self, channels, gen_scale=10., trans_scale=0.1,
                epsilon=1e-3):
        super(ChannelUniformAug, self).__init__()

        self.trans_scale = trans_scale

        self.width = nn.Parameter(torch.zeros(channels, 6))
        self.softplus = torch.nn.Softplus()
        self.g0 = None
        self.std_batch_size = None
    ...
    def set_width(self, vals):
        with torch.no_grad():
            self.width.copy_(vals[None].expand_as(self.width))
        
    def transform(self, x):
        bs, chan, w, h = x.size()
        weights = torch.rand(bs, chan, 6)
        weights = weights.to(x.device, x.dtype)
        width = self.softplus(self.width)
        weights = weights * width - width.div(2.)

        # (bs * chan, 3, 3)
        generators = self.generate(weights.view(bs * chan, 6))

        affine_matrices = expm(generators)

        x = x.view(bs * chan, 1, w, h)
        flowgrid = F.affine_grid(affine_matrices[:, :2, :], size = x.size(),
                                 align_corners=True)
        x_out = F.grid_sample(x, flowgrid,align_corners=True)
        x_out = x_out.view(bs, chan, w, h)
        return x_out
